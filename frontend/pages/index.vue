<template class="container">
  <section>
      ...
      <input v-model="findName" id="filtername" type="text" v-on:keydown="filteredContact"/>
      <ul v-if="filteredNames">
          <li v-for="(item, id) in names" :key="id"  >
              {{ item }}
          </li>
      </ul>
      &lt;!&ndash;<input type="input" v-on:keydown="searchContact">
      <div id="search">
      ...
      <table>
          <tr>
              ...
              <th v-if="editButton==true">Delete contact</th>
              <th v-if="editButton==true">Delete from database</th>
          </tr>
          <tr v-for="(item, id) in phonebook"
              :key="id" v-if="item.deleted===0">
              <td> {{ item.first_name }}</td>
              <td>{{ item.last_name }}</td>
              <td>{{ item.telephone_number }}</td>
              <td v-if="editButton==true"><button v-on:click="deleteContact(item.id)" class="btn" >Delete contact</button></td>
              <td v-if="editButton==true"><button @click="deleteFromDataBase(item.id)" class="btn" >Delete contact from db</button></td>
          </tr>
      </table>-->
      ...
      <aside id="colorlib-hero">
          ...
          <div class="flexslider" id="slider-image">

              <ul class="slides">
                  <!--<s-slider-image
                          v-for="(image,id) in sliderList"
                          :slider="image"
                          :data-id="id"
                          :key="id"
                  ></s-slider-image>-->
          ...
          <div class="container">
              <div class="row">
                  <div class="col-md-6 col-md-offset-3 text-center colorlib-heading">
                      <h2><span>Izdvajamo iz ponude</span></h2>
      ...
      <div id="colorlib-intro" class="colorlib-intro" style="background-image: url(images/posao.jpg);" data-stellar-background-ratio="0.5">
                              ...
                              <div class="text-lefts">
                                  <div class="sale-box">
                                      <div class="sale-box-top">
                                          <h2 class="number">Tražite</h2>
                                          <!--<span class="sup-1">%</span>
                                          <span class="sup-2">Off</span>-->
                                      </div>
                                      <h2 class="text-sale">Posao?</h2>
                                  </div>
                              </div>
                              <div class="text-rights">
                                  <h3 class="title">Kontaktirajte nas!</h3>
                                  <!--<p>Pošaljite nam Vaš CV i propratno pismo na mail: <h4>vpyusorbon@gmail.com</h4></p>-->
                                  <p><a href="shop.html" class="btn btn-primary">Konkuriši odmah</a> <a href="#" class="btn btn-primary btn-outline">Pročitaj detaljnije</a></p>
                              </div>
                          </div>
                      </div>
                  </div>
          ...
          <div class="container">
              <div class="row">
                  <div class="col-md-8 col-md-offset-2 text-center colorlib-heading">
                      <h2>Novosti</h2>
                      <p>Najnovije vesti kompanije YU SORBON</p>
                  </div>
              </div>
...
export default {
    ...
    computed:{
        filteredNames: function (){
            ...
            return this.phonebook.filter(el => el.match(filter))
        }
    },
    methods:{
        /*filteredContact() {

            var filtered = this.phonebook
            this.activePhonebook.forEach(filter1 => {
                filtered = filter1.last_name
            })

            filter(record => {
    return filter.last_name === 'last_name'
        ? new RegExp(filter.value, 'i').test(record[filter.last_name])
        : record[filter.last_name] == filter.value
        })
        })*/
        return filtered
        console.log(filtered.stringify(this.filtered.last_name))

            let result = JSON.stringify(this.phonebook)
            let r = result.slice(23,27)
            console.log(r)

            let filter1 = new RegExp(this.findName, 'i')
            let p = this.phonebook.forEach(item => {
                this.activePhonebook = item.last_name
            })
            return this.activePhonebook.filter(el => el.match(filter1))

        },
        deleteContact: function (id){
            return axios.post('http://localhost:8000/api/deletecontact',{
                    id: id,
                    deleted: '1'
                }
            )},
        ...
        /*searchContact: function(){
            ...
            /!*id.innerHTML =
                '<ul>' +
                '<li>' + this.phonebook.last_name + '</li>' +
                '</ul>>'*!/
        }*/
    },
  components: {
